...
class TestDateValidation:
    ...
    def test_date_without_operator_treated_as_keyword(self) -> None:
        """Test that numbers without operators are treated as keywords or result count."""
        result = self.parser.parse("quantum 20240101")
        
        assert result.success
        assert result.search is not None
        # Should be treated as keywords, not a date
        assert "ti:quantum AND ti:20240101" in result.search.query
...
class TestSchedulerCompatibility:
    ...
    def test_long_topic_with_dates(self) -> None:
        ...
        assert result.success
        assert result.search is not None
        assert "ti:neural AND ti:networks AND ti:deep AND ti:learning AND ti:transformers" in result.search.query
        assert "submittedDate:[20240101000000 TO 20250101000000]" in result.search.query