...
class TestPhase2Features:
    ...
    def test_not_operator(self):
        ...
        assert result.success
        assert result.query_string == "ti:quantum AND NOT ti:classical"

    def test_not_with_prefix(self):
        """Test NOT operator with prefixed terms."""
        result = self.parser.parse("quantum -@smith")
        assert result.success
        assert result.query_string == "ti:quantum AND NOT au:smith"
    ...
    def test_mixed_operators(self):
        ...
        assert result.success
        # Should be: (ti:quantum OR ti:neural AND NOT ti:classical AND au:hinton)
    ...
    def test_operator_precedence(self):
        ...
        assert result.success
        # Our implementation: (quantum OR neural AND NOT classical AND deep)
        assert "ti:quantum OR" in result.query_string
        assert "NOT ti:classical" in result.query_string
        assert "ti:deep" in result.query_string
