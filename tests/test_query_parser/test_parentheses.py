...
class TestParenthesesWithFieldPrefixes:
    ...
    def test_field_prefix_with_parentheses(self) -> None:
        """Test field prefix applied to parentheses group."""
        result = self.parser.parse("@(einstein bohr)")
        
        assert result.success
        assert result.search is not None
        assert result.search.query == "au:(ti:einstein AND ti:bohr)"

    def test_category_with_parentheses(self) -> None:
        """Test category prefix with parentheses."""
        result = self.parser.parse("#(physics quantum)")
        
        assert result.success
        assert result.search is not None
        # Should apply category to the grouped terms
        assert "cat:" in result.search.query
        assert "physics" in result.search.query
        assert "quantum" in result.search.query

    def test_abstract_with_parentheses(self) -> None:
        """Test abstract prefix with parentheses."""
        result = self.parser.parse("$(machine learning)")
        
        assert result.success
        assert result.search is not None
        assert result.search.query == "abs:(ti:machine AND ti:learning)"
    ...
    def test_arxiv_field_with_parentheses(self) -> None:
        ...
        assert result.success
        assert result.search is not None
        assert result.search.query == "ti:(ti:quantum AND ti:computing)"

    def test_field_prefix_with_or_in_parentheses(self) -> None:
        """Test field prefix with OR operator in parentheses."""
        result = self.parser.parse("@(einstein | bohr)")
        
        assert result.success
        assert result.search is not None
        assert result.search.query == "au:(ti:einstein OR ti:bohr)"
