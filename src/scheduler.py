            ...
            for guild in guilds:
                ...
                auto_channels.extend(
                    [
                        ...
                        if channel.get("type") == 0 and channel.get("name", "").startswith("auto")
                    ],
                )
                ...
                try:
                    ...
                    if not topic:
                        ...
                        await self.send_message(
                            channel_id,
                            "No search query found in channel topic",
                        )
                        continue
                    ...
                    if search_query is None:
                        logger.warning(
                            "Invalid query for channel %s: %s",
                            channel["name"],
                            query_text,
                        )
                        await self.send_message(
                            channel_id,
                            "Invalid query format in channel topic",
                        )
                        continue
